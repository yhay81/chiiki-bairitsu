const MAX_COMPARE = 4;
const STORAGE_KEY = "chiiki-bairitsu:compare:v1";
...
const getSession = () => {
  const key = "chiiki-bairitsu:session:v1";
...
const track = (name) => {
  ...
  fetch("/api/telemetry", {
    ...
    headers: {
      ...
      "x-chiiki-bairitsu-session": getSession(),
      "x-chiiki-bairitsu-qa": isQa() ? "1" : "0",
    },
...
const valuesFor = (placeId, yearIndex = currentYearIndex()) => {
  ...
  if (!pair) return { openings: null, seekers: null, ratio: null };
  const [openings, seekers] = pair;
  return { openings, seekers, ratio: seekers > 0 ? openings / seekers : null };
};
...
function ratioDomain() {
  const values = records
    .flatMap((record) => record[employment.value].map(([openings, seekers]) => openings / seekers))
...
function sparkline(placeId) {
  const record = recordFor(placeId);
  const domain = ratioDomain();
  const ratios = record[employment.value].map(([openings, seekers]) => openings / seekers);
  const points = ratios
    .map((value, i) => {
  ...
  return `<svg aria-label="2023年度から2025年度の求人倍率推移" class="sparkline" role="img" viewBox="0 0 100 62">
  ...
  compareList.innerHTML = places
    .map((place) => {
      ...
      return `<article class="compare-card">
        ...
        <div class="compare-chart">${ratioDial(values.ratio, `${place.name}の求人倍率 ${formatRatio(values.ratio)}`)}${sparkline(place.id)}</div>
        <div class="year-scale"><span>2023</span><span>${currentYear()}</span><span>2025</span></div>
        <dl class="count-pair">
          <div><dt>月間有効求人数</dt><dd>${formatCount(values.openings)}</dd></div>
          <div><dt>月間有効求職者数</dt><dd>${formatCount(values.seekers)}</dd></div>
  ...
  results.innerHTML = visible
    .map((place) => {
      ...
      return `<article class="place-card">
        ...
        ${balanceBar(values.ratio, `${place.name} ${currentYear()}年度の求人倍率 ${formatRatio(values.ratio)}`)}
...
copyCompare.addEventListener("click", async () => {
  const lines = selected
    ...
    .map((place) => {
      const values = valuesFor(place.id);
      return `${place.name}｜${formatRatio(values.ratio)}｜月間有効求人 ${formatCount(values.openings)}｜月間有効求職 ${formatCount(values.seekers)}｜前年差 ${formatChange(ratioChange(place.id))}`;
    });
  await navigator.clipboard.writeText(
    [
      `地域求人倍率（${currentYear()}年度・${currentEmployment().name}・職業計）`,
      ...lines,
      "月間有効求人数÷月間有効求職者数。採用確率・仕事の質・地域順位ではありません。",
