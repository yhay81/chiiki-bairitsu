...
for (const record of records) {
  for (const employment of index.employments) {
    ...
    for (const [opening, seeker] of pairs) {
      assert(Number.isInteger(opening) && opening >= 0, `opening ${record.p}`);
      assert(Number.isInteger(seeker) && seeker > 0, `seeker ${record.p}`);
    }
  }
...
assert(national.a[2][0] === 24_988_674, "known nationwide openings");
assert(national.a[2][1] === 22_698_922, "known nationwide seekers");
const tokyo = records.find((record) => record.p === "JP-13");
assert(tokyo.a[2][0] === 3_747_553 && tokyo.a[2][1] === 2_456_025, "known Tokyo values");
console.log(
