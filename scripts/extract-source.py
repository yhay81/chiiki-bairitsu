"""Extract matched occupation-total values from MHLW annual tables 4 and 5."""
...
def load_seekers(path: Path, place_ids: dict[str, str]) -> dict[tuple[str, str], list[int | None]]:
    book = openpyxl.load_workbook(path, read_only=True, data_only=True)
    totals: dict[tuple[str, str], list[int | None]] = {}
    try:
        for sheet_index, employment in SHEETS.items():
...
def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit("usage: extract-source.py OPENINGS.xlsx SEEKERS.xlsx OUTPUT_DIRECTORY")
    ...
    seekers = load_seekers(seekers_path, place_ids)
    expected_series = len(places) * len(EMPLOYMENTS)
    if len(openings) != expected_series or len(seekers) != expected_series:
        raise ValueError(f"unexpected source series: {len(openings)=} {len(seekers)=}")

    records: list[dict[str, object]] = []
    pair_count = 0
    employment_checks = {"openings": 0, "seekers": 0}
    national_checks = {"openings": 0, "seekers": 0}
    ...
    for place in places:
        ...
        for employment in EMPLOYMENTS:
            ...
            pairs = []
            for opening, seeker in zip(openings[(place["id"], employment_id)], seekers[(place["id"], employment_id)], strict=True):
                if opening is None or seeker is None or opening < 0 or seeker <= 0:
                    raise ValueError(f"invalid value: {place['id']} {employment_id}")
                pairs.append([opening, seeker])
                pair_count += 1
    ...
    index = {
        ...
        "sources": [
            ...
            {"kind": "seekers", "url": SEEKERS_URL, "bytes": seekers_path.stat().st_size, "sha256": sha256(seekers_path)},
        ],
    }
    ...
    print(json.dumps({"pairs": pair_count, "places": len(places), "source_values": pair_count * 2, "openings_sha256": index["sources"][0]["sha256"], "seekers_sha256": index["sources"][1]["sha256"]}, ensure_ascii=False))
