...
describe("product surface", () => {
  it("communicates the regional balance with tickets, tracks, a ratio board, and trends", () => {
    expect(worker).toContain('class="flow-ticket opening-ticket"');
    expect(worker).toContain('class="flow-ticket seeker-ticket"');
    expect(worker).toContain('class="ratio-board"');
  ...
  it("states formula, definitions, and interpretation boundaries", () => {
    expect(worker).toContain("月間有効求人数 ÷ 月間有効求職者数");
    expect(worker).toContain("複数月に含まれる");
  ...
  it("renders four indexable pages with constrained typography and no meta copy", async () => {
    for (const path of ["/", "/guide", "/source", "/privacy"]) {
      const response = await app.request(
        `https://chiiki-bairitsu.yhay81.com${path}`,
        undefined,
        bindings,
      );
  ...
  it("accepts only same-origin allowlisted telemetry", async () => {
    const accepted = await app.request(
      "https://chiiki-bairitsu.yhay81.com/api/telemetry",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
          origin: "https://chiiki-bairitsu.yhay81.com",
          "x-chiiki-bairitsu-session": "12345678-1234-4123-8123-123456789abc",
          "x-chiiki-bairitsu-qa": "1",
        },
  ...
  it("separates QA, honors privacy signals, and needs no account", () => {
    ...
    expect(client).toContain('"x-chiiki-bairitsu-qa"');
